...
function initializePageList() {
	...
	$(window).on("keydown", function(e) {
		if (e.keyCode == 190) {
			...
			if (on == true) {
				$("#PageViewer").css({
					top: $("#PageViewer").position().top - 200
				});
			}
			...
			if (on == true) {
				$("#PageViewer").css({
					left: $("#PageViewer").position().left - 200
				});
			}
...
function gotoPage(pos)
{
	$("#PageViewer").css("opacity", 0);
						showSpinner();
						var img = new Image();
						$.post("core/getPage.php", "mid=" + $(".current").data("mid") + "&position=" + (pos), function(data, status) {
							var img = new Image();
...
function ShowPageContainer(obj, mid, p) {
	...
	$.post("core/getPage.php", "mid=" + $(obj).data("mid") + "&position=" + p, function(data, status) {
		...
		bg.onload = function() {
			...
			on = true;
			$("#PageViewer").attr("src", img.src);
			var PageViewerWidth=((document.documentElement.clientHeight - 60)/$(".current").height())*$(".current").width();
			$("#PageViewer").css({
					left: (document.documentElement.clientWidth - PageViewerWidth) / 2,
			...
			$("#PageViewer").on("dblclick", function(event) {
				...
				$("#PageViewer").css({
					top: $("#PageViewer").offset().top + dy,
					left: $("#PageViewer").position().left + dx
				});
			...
			$("#ShowPageContainer").on("dblclick", function(event) {
				...
				$("#PageViewer").css({
					top: $("#PageViewer").position().top + dy,
					left: $("#PageViewer").position().left + dx
				});
...
function HidePageContainer() {
	...
	position = 0;
	on = false;
	var goUp=$(".current").data("goup");
	$("#PageViewer").css({
		left: $(".current").position().left,
		height: $(".current").height(),
			top: $(".current").position().top + goUp
	});
	$("#Background").css({
		display: "block"
	}).css({
...
function magnify(ratio) {
	...
	$('#PageViewer').css({
		height: height * ratio,
		left: $('#PageViewer').position().left - dw,
		top: $('#PageViewer').position().top - dh
	});
}
